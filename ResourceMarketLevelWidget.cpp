...
{
    int leftIndex = level->GetCapacity(resource) - level->GetCounter(resource);

    for (int i = 0; i < level->GetCapacity(resource); i++)
    {
        auto halfOpacity = new QGraphicsOpacityEffect(this);
        halfOpacity->setOpacity(0.5);
        auto fullOpacity = new QGraphicsOpacityEffect(this);
        ...
        {
            case COAL:
                button = coalButtons[i];
                break;
            case OIL:
                button = oilButtons[i];
                break;
            case GARBAGE:
                button = garbageButtons[i];
                break;
            case URANIUM:
                button = uraniumButtons[i];
                break;
            default:
                button = new QPushButton(this);     // Just to cover all cases!
                break;
        }

        if (i < leftIndex || amount == 0)
            button->setGraphicsEffect(halfOpacity);
        else if (amount > 0)
        {
            button->setGraphicsEffect(fullOpacity);
            amount--;
        }
    }
}
